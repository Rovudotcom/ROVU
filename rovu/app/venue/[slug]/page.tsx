...
export default function VenuePage() {
  ...
  function goToReview() {
    ...
    else if(selStaff.length>1){const last=selStaff[selStaff.length-1];const rest=selStaff.slice(0,-1).join(', ');staffLine=` Special mention to ${rest} and ${last}.`}
    const openers: Record<number,string> = {5:"We had an absolutely wonderful evening. ",4:"Had a really enjoyable dinner and will definitely be back. ",3:"It was a decent experience overall, though a few areas could be improved. ",2:"Unfortunately our visit was quite disappointing. ",1:"Had a very poor experience and cannot recommend it. "}
    const closers: Record<number,string> = {5:" Highly recommend — we will definitely be back very soon!",4:" Overall a very solid restaurant.",3:" Worth a visit nearby but needs some improvements.",2:" Hopefully management takes this on board.",1:" We will not be returning."}
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex flex-col max-w-sm mx-auto">
      ...
      <div className="border-b border-[#1e1e1e] px-6 pt-14 pb-5 flex items-center justify-between">
        ...
        <Link href="/" className="text-white/30 text-xs border border-white/10 px-3 py-1.5 rounded-lg hover:text-white/60 transition-all">Back to Rovu</Link>
      </div>

      {!['gate','thanks'].includes(screen)&&(
        <div className="flex gap-1.5 justify-center pt-5 pb-1">
          {['rate','chips','staff','review'].map(s=><div key={s} className={`h-1.5 rounded-full transition-all duration-300 ${s===screen?'bg-[#5B4FE8] w-4':'bg-white/15 w-1.5'}`}/>)}
        </div>
      )}

      {showBack&&(
        <button onClick={goBack} className="text-white/35 text-sm px-6 pt-3 pb-1 text-left hover:text-white/60 transition-all">← Back</button>
      )}
      ...
      <div className="flex-1 px-6 py-5 overflow-y-auto">
            ...
            <div>
              ...
              <div className="flex gap-2">
                {PLATFORMS.map(p=><button key={p.key} onClick={()=>togglePlat(p.key)} className={`flex-1 py-3 rounded-2xl border text-sm font-medium transition-all flex flex-col items-center gap-1 ${selPlats.includes(p.key)?'border-[#5B4FE8] bg-[#5B4FE8]/10 text-white':'border-white/10 text-white/40 hover:border-white/25'}`}>
                  <span className="text-base font-bold" style={{color:selPlats.includes(p.key)?'#fff':p.color}}>{p.icon}</span>
        ...
        {screen==='gate'&&(
          <div className="flex flex-col items-center text-center pt-8">
            ...
            <p className="text-white/45 text-sm leading-relaxed mb-6">We are sorry your experience was not perfect. Your feedback has been sent directly to the management team.</p>
            ...
            <div className="flex flex-col gap-3 w-full">
              <button onClick={reset} className="w-full py-3 bg-[#5B4FE8] text-white rounded-2xl text-sm font-medium hover:opacity-80 transition-all">Rate another visit</button>
              <Link href="/" className="w-full py-3 text-center border border-white/10 text-white/40 rounded-2xl text-sm hover:border-white/20 hover:text-white/60 transition-all">Back to Rovu</Link>
            </div>
          </div>
        )}
