...
export default function DashboardPage({ params }: { params: { slug: string } }) {
    ...
    async function load() {
      ...
      setStaffMentions(
        Object.entries(mentions)
          .map(([name, count]) => ({ name, count, initials: name.split(' ').map((w:string) => w[0]).join('').slice(0,2) }))
          .sort((a, b) => b.count - a.count)
      )
      // Generate QR code
      const url = `${window.location.origin}/venue/${v.slug}`
  ...
  function downloadQR() {
    ...
    a.href = qrUrl
    a.download = `rovu-qr-${venue.name.toLowerCase().replace(/\s+/g,'-')}.png`
    a.click()
  }
  ...
  const card = {background:'#0f0f0f',border:'.5px solid rgba(255,255,255,.07)',borderRadius:14,padding:'1.125rem 1.25rem'}

  return (
    <div style={{minHeight:'100vh',background:'#0a0a0a',color:'#fff',fontFamily:'Inter,system-ui,sans-serif'}}>
      <nav style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'.875rem 1.75rem',borderBottom:'.5px solid rgba(255,255,255,.07)',background:'rgba(10,10,10,.97)',position:'sticky',top:0,zIndex:10}}>
      ...
      {/* Tabs */}
      <div style={{display:'flex',gap:0,borderBottom:'.5px solid rgba(255,255,255,.07)',padding:'0 1.75rem',background:'#0a0a0a'}}>
        {(['overview','qr'] as const).map(t => (
          <button key={t} onClick={()=>setTab(t)} style={{padding:'10px 16px',background:'none',border:'none',borderBottom:`.5px solid ${tab===t?'#5B4FE8':'transparent'}`,color:tab===t?'#fff':'rgba(255,255,255,.35)',fontSize:13,fontWeight:tab===t?500:400,cursor:'pointer',marginBottom:-1,letterSpacing:'.01em',textTransform:'capitalize'}}>
            {t==='qr'?'QR Code':t}
          </button>
        ))}
      </div>

      <div style={{padding:'1.5rem 1.75rem',maxWidth:1140,margin:'0 auto'}}>
          ...
          <>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,marginBottom:14}}>
              {[
                ...
                <div key={m.label} style={{...card,paddingTop:'1.125rem',position:'relative',overflow:'hidden'}}>
                    ...
                    return(
                      <div key={r} style={{display:'flex',alignItems:'center',gap:8,marginBottom:7}}>
                        ...
                        <div style={{flex:1,height:5,background:'rgba(255,255,255,.06)',borderRadius:3,overflow:'hidden'}}>
                          <div style={{height:'100%',borderRadius:3,width:`${pct}%`,background:r>=4?'#5B4FE8':'#E8934F',opacity:r>=4?1:r===3?.7:.5}}></div>
                        </div>
        ...
        {tab==='qr' && (
          <div style={{maxWidth:480,margin:'0 auto',paddingTop:'2rem',textAlign:'center'}}>
            <div style={{fontSize:20,fontWeight:500,letterSpacing:'-.03em',marginBottom:6}}>your QR code</div>
            <div style={{fontSize:13,color:'rgba(255,255,255,.35)',marginBottom:'2rem'}}>print this and place it on every table</div>
            {qrUrl ? (
              <>
                <div style={{background:'#fff',borderRadius:20,padding:'1.5rem',display:'inline-block',marginBottom:'1.5rem'}}>
                  <img src={qrUrl} alt="QR Code" style={{width:220,height:220,display:'block'}}/>
                  <div style={{marginTop:'1rem',textAlign:'center'}}>
                    <div style={{fontSize:13,fontWeight:500,color:'#0a0a0a',marginBottom:2}}>{venue.name}</div>
                    <div style={{fontSize:11,color:'rgba(0,0,0,.4)'}}>scan to leave a review</div>
                  </div>
                </div>
                <div style={{marginBottom:'1rem'}}>
                  <div style={{fontSize:11,color:'rgba(255,255,255,.3)',marginBottom:'1rem',background:'rgba(255,255,255,.04)',border:'.5px solid rgba(255,255,255,.08)',borderRadius:8,padding:'8px 14px',display:'inline-block'}}>
                    {typeof window !== 'undefined' ? `${window.location.origin}/venue/${params.slug}` : ''}
                  </div>
                </div>
                <button onClick={downloadQR} style={{background:'#5B4FE8',color:'#fff',border:'none',borderRadius:11,padding:'13px 32px',fontSize:14,fontWeight:500,cursor:'pointer',letterSpacing:'.01em'}}>
                  download PNG →
                </button>
                <div style={{fontSize:12,color:'rgba(255,255,255,.25)',marginTop:'1rem'}}>print at A5 or A6 size for best results</div>
              </>
            ) : (
              <div style={{fontSize:13,color:'rgba(255,255,255,.3)'}}>generating QR code...</div>
            )}
          </div>
        )}
      </div>
